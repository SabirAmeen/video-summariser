...
export function UploadSection({
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="group relative mt-2 flex justify-center rounded-2xl border-2 border-dashed border-zinc-800 px-6 py-16 transition-all duration-300 hover:border-indigo-500/50 hover:bg-zinc-900/50 cursor-pointer">
        <input
          type="file"
          className="hidden"
        ...
        <div className="text-center space-y-4">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-zinc-900 group-hover:bg-indigo-500/10 transition-colors duration-300 border border-zinc-800 group-hover:border-indigo-500/20">
            {isUploading ? (
              ...
              <UploadCloud
                className="h-8 w-8 text-zinc-400 group-hover:text-indigo-400 transition-colors duration-300"
                aria-hidden="true"
              />
            )}
          </div>
      ...
      {uploadStatus && (
        <div
          className={`text-sm text-center font-medium ${uploadStatus.includes("successful") ? "text-emerald-500" : "text-rose-500"}`}
        >
          {uploadStatus}
        </div>
      )}
